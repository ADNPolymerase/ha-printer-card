...
| `image` | Une photo de votre propre machine à la place du dessin : une URL, ou un chemin comme `/local/xp7100.jpg`. Elle est ajustée dans une case fixe de 3:2, donc jamais rognée, jamais déformée, et la carte garde sa hauteur quel que soit le format fourni. Tout ce qui va du 4:3 au 16:9 remplit cette case ; une image carrée ou un panorama s'affiche simplement plus petit au centre. Elle ne peut pas s'animer, rien ici ne sachant où se trouve la sortie papier de cette imprimante, mais elle porte quand même l'état : un bourrage ou un avertissement y pose un triangle, une imprimante hors ligne est atténuée, et une imprimante qui imprime reçoit un anneau lent dans la couleur de l'état. Un chemin qui ne charge pas retombe sur le dessin. Avec une photo, `cartridge_style: inside` n'est pas disponible et les cartouches passent en dessous. Sur un thème sombre, un PNG à fond transparent rend bien mieux qu'un JPEG sur blanc. |
