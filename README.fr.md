...
- **Les imprimantes photo comprises.** Huit encres sur un Canon PRO-100, dix sur un Epson grand format : cyan photo, magenta photo, gris clair, noir mat, noir clair et noir très clair sont distingués de leurs homologues simples, par leur nom complet ou par le code court imprimé sur la cartouche (`pk`, `mk`, `lgy`, `vlm`). À l'autre bout, une **cartouche trois couleurs** est reconnue au fait qu'elle nomme plusieurs primaires à la fois, quelle que soit l'écriture de l'imprimante (`CyanMagentaYellow`, `Cyan/Magenta/Yellow`, `CMY`), et elle est dessinée avec ses trois chambres côte à côte plutôt que dans une teinte inventée.
- **Des consommables trouvés tout seuls.** Chaque pourcentage posé sur l'appareil de l'imprimante est un candidat, moins les capteurs Wi-Fi et de diagnostic que les intégrations accrochent au même appareil. Ils sont ensuite triés en **encres** (dessinées dans leur couleur, du noir au jaune) et en **pièces d'usure** (tambour, courroie, four, unité laser, kits d'alimentation, boîte de maintenance) affichées en pastilles compactes, chacune **nommée par la carte et non par l'intégration** : un Brother en français annonce son four en « Durée de vie restante de l'unité de fusion », la carte écrit *Four*, dans la langue où elle est réglée.
...
- **Compteurs, une ligne par fonction** : impression, numérisation, copie, fax, chacune nommée et chacune avec son total et sa ventilation noir et blanc / couleur quand l'imprimante la donne. Les bourrages et les estimations de pages restantes restent dehors. `counters_align` centre le bloc sur une machine qui ne fait qu'imprimer.
...
- **Les motifs de l'imprimante, dans la langue de la carte.** Les motifs RFC 8011 sont une liste finie, donc dix-neuf des plus courants sont traduits : `media-empty` s'affiche *Plus de papier*, `marker-supply-low-warning` s'affiche *Encre faible*. Leur gravité vient du jeton et non des mots, donc un bourrage arrête l'imprimante et un suffixe `-warning` se contente d'avertir, dans les treize langues à la fois. Une imprimante en envoie plusieurs à la fois, séparés par des virgules : tous sont nommés et le plus grave décide, un `-report` purement informatif est écarté quand autre chose demande une action, et gardé quand c'est tout ce que l'imprimante a à dire. Un avertissement de consommable n'efface plus le fait que la machine imprime. Ce qui n'est pas dans la liste reste rendu lisible.
...
| `cartridges` | Capteurs de consommables. Sans cette option, ils sont détectés : même appareil, ou même préfixe d'`entity_id`. Chaque entrée est un identifiant d'entité, ou `{entity, name, color, kind, ref}` pour les imprimantes dont les capteurs ne portent que la référence. `ref` est la référence à racheter, affichée au survol et passée à `shop_url`. |
| `counters` | Compteurs de pages, un identifiant d'entité par ligne. Sans cette option, ils sont détectés de la même façon. Les nommer, c'est les imposer : un compteur à l'unité inattendue, ou qui vit hors de l'appareil de l'imprimante parce qu'il vient d'un capteur template, est pris tel quel. Leur nom sert toujours à savoir de quelle fonction ils relèvent et s'ils comptent la couleur ou le noir. |
| `extra_entities` | Tout ce qui mérite en plus une place sur la carte, une entité par ligne, affiché sous l'état dans l'ordre donné : une durée de fonctionnement, une version de firmware, un niveau wifi. Rien n'est détecté ici, contrairement aux cartouches et aux compteurs : ce que vous listez est ce que vous obtenez. Un nombre est affiché dans votre langue avec son unité, un `timestamp` en date, et une entité qui n'existe pas est ignorée plutôt que dessinée vide. Écrivez `{ entity: sensor.x, name: Uptime }` pour imposer le libellé ; sinon c'est le nom de l'entité, débarrassé du nom de l'imprimante en tête. |
...
| `shop_url` | Où racheter une cartouche. `{ref}` est remplacé par la `ref` du consommable, donc une seule adresse couvre toutes les encres : `https://www.example.com/search?q={ref}`. Un consommable qui a à la fois une référence et cette adresse ouvre la boutique au clic plutôt que sa propre entité ; les autres ne changent pas. `http` et `https` uniquement. |
...
| `image` | Une photo de votre propre machine à la place du dessin : une URL, ou un chemin comme `/local/xp7100.jpg`. Utilisez une image carrée : la case est carrée, c'est le format des photos produit, donc une image carrée la remplit d'un bord à l'autre. Tout autre format est centré dedans, jamais rogné, jamais déformé, et la carte garde sa hauteur dans les deux cas. Elle ne peut pas s'animer, rien ici ne sachant où se trouve la sortie papier de cette imprimante, mais elle porte quand même l'état : un bourrage ou un avertissement y pose un triangle en plein milieu, une imprimante hors ligne est atténuée, et une imprimante qui imprime reçoit un anneau lent autour, dans la couleur de l'état. Un chemin qui ne charge pas retombe sur le dessin. Avec une photo, `cartridge_style: inside` n'est pas disponible et les cartouches passent en dessous. Sur un thème sombre, un PNG à fond transparent rend bien mieux qu'un JPEG sur blanc. |
| `printer_type` | `mfp` (défaut), `inkjet`, `laser` ou `office`. |
| `cartridge_order` | `color` (défaut) garde l'ordre imprimé sur la machine, du noir au jaune. `level` met les plus vides en premier, ce qu'on veut quand la carte est là pour dire quoi racheter. Un consommable qui n'annonce aucun niveau n'a rien sur quoi être trié et passe en dernier. |
| `counters_align` | `left` (défaut) ou `center`. `center` ne déplace que les chiffres : les noms de fonction gardent leur colonne à gauche, donc les lignes restent alignées entre elles quelle que soit leur longueur. |
...
| `name_position` | `below` (défaut) garde le nom sous l'illustration. `top` le remonte dans une ligne d'en-tête en haut à gauche, où il partage la ligne avec la prise et sa consommation. Le mode compact pose déjà le nom à côté de l'icône et ignore l'option. |
