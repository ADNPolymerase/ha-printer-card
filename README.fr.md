...
| `shop_url` | Où acheter les cartouches. Dès qu'elle est renseignée, un panier discret apparaît en haut à droite, à côté de la prise. `{ref}` est remplacé par le mot cartouches dans la langue de la carte, suivi de la marque et du modèle lus dans le registre d'appareils et non du nom que vous avez donné à la carte : `https://www.example.com/search?q={ref}` devient une recherche de `Cartouches Canon TS8050 series`. Un modèle sans `{ref}` est pris tel quel, donc un lien direct vers la page de votre machine fonctionne aussi. Le clic sur une cartouche ouvre toujours son historique. `http` et `https` uniquement. |
