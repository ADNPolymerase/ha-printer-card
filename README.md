...
| `counters_align` | `left` (default) or `center`. With several functions the counters form a table with a label column, and left is where it belongs. A machine that only prints has one bare row, which floats under a centred cartridge row; `center` puts it back under them. |
