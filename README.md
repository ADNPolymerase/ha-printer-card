...
- **The printer's reasons, in the card's language.** RFC 8011 state reasons are a closed list, so nineteen of the common ones are translated: `media-empty` reads *Out of paper*, `marker-supply-low-warning` reads *Ink low*. Their severity comes from the token rather than the wording, so a jam stops the printer and a `-warning` suffix only warns, in all thirteen languages at once. Anything unlisted is still made readable.
