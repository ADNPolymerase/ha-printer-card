...
| `shop_url` | Where to buy cartridges. Set it and a discreet basket appears in the top right corner, next to the socket. `{ref}` is replaced by the word for cartridges in the card's language followed by the printer's make and model, taken from the device registry rather than from the name you gave the card: `https://www.example.com/search?q={ref}` becomes a search for `Cartridges Canon TS8050 series`. A template with no `{ref}` is used as it stands, so a direct link to the page for your own machine works too. Tapping a cartridge still opens its history. `http` and `https` only. |
