...
| `image` | A photo of your own machine instead of the drawing: a URL, or a path such as `/local/xp7100.jpg`. It is fitted into a fixed 3:2 slot, so it is never cropped, never stretched, and the card keeps its height whatever shape you feed it. Anything between 4:3 and 16:9 fills that slot; a square or a panorama simply sits smaller in the middle. It cannot animate, since nothing here knows where that printer's output slot is, but it still carries the state: a jam or a warning puts a triangle on it, an offline printer is dimmed, and a printing one gets a slow ring in the state colour. A path that fails to load falls back to the drawing. With a photo, `cartridge_style: inside` is not available and the cartridges move below. On a dark theme, a PNG with a transparent background looks far better than a JPEG on white. |
