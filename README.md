...
- **Photo printers included.** Eight inks on a Canon PRO-100, ten on a wide format Epson: photo cyan, photo magenta, light grey, matte black, light black and light light black are told apart from their plain counterparts, by full name or by the short code printed on the cartridge (`pk`, `mk`, `lgy`, `vlm`). At the other end, a **tri-colour cartridge** is recognised by the fact that it names several primaries at once, however the printer spells it (`CyanMagentaYellow`, `Cyan/Magenta/Yellow`, `CMY`), and is drawn with its three chambers side by side rather than in one invented colour.
- **Supplies found on their own.** Every percentage on the printer's device is a candidate, minus the Wi-Fi and diagnostic sensors integrations hang off the same device. They are then sorted into **inks** (drawn in their own colour, black to yellow) and **wear parts** (drum, belt, fuser, laser unit, feed kits, maintenance box) shown as a compact chip row, each **named by the card rather than by the integration**: a French Brother reports its fuser as "Durée de vie restante de l'unité de fusion", which reads *Four* on the card, in whatever language the card is set to.
...
- **Counters, one row per function**: printed, scanned, copied, faxed, each named and each with its total, black-and-white and colour split when the printer reports one. Jams and remaining-page estimates are counters too and stay out. `counters_align` centres the block on a machine that only prints.
...
- **The printer's reasons, in the card's language.** RFC 8011 state reasons are a closed list, so nineteen of the common ones are translated: `media-empty` reads *Out of paper*, `marker-supply-low-warning` reads *Ink low*. Their severity comes from the token rather than the wording, so a jam stops the printer and a `-warning` suffix only warns, in all thirteen languages at once. A printer sends several reasons at once, comma separated, so all of them are named and the most severe decides; an informational `-report` is dropped when something else needs doing, and kept when it is all the printer has to say. A supply warning no longer erases the fact that the machine is printing. Anything unlisted is still made readable.
...
| `cartridges` | Supply sensors. Leave it out and they are discovered: same device, or same `entity_id` prefix. Each item is an entity id, or `{entity, name, color, kind, ref}` for printers whose sensors are named after the part number only. `ref` is the part number to reorder, shown on hover and handed to `shop_url`. |
...
| `shop_url` | Where to buy a cartridge. `{ref}` is replaced by that supply's `ref`, so one address covers every ink: `https://www.example.com/search?q={ref}`. A supply that has both a reference and this address opens the shop when tapped instead of its own entity; every other supply is unchanged. `http` and `https` only. |
...
| `image` | A photo of your own machine instead of the drawing: a URL, or a path such as `/local/xp7100.jpg`. Use a square picture: the slot is square, which is the shape product shots come in, so a square one fills it edge to edge. Any other shape is centred inside it instead, never cropped, never stretched, and the card keeps its height either way. It cannot animate, since nothing here knows where that printer's output slot is, but it still carries the state: a jam or a warning puts a triangle across the middle of it, an offline printer is dimmed, and a printing one gets a slow ring around it in the state colour. A path that fails to load falls back to the drawing. With a photo, `cartridge_style: inside` is not available and the cartridges move below. On a dark theme, a PNG with a transparent background looks far better than a JPEG on white. |
| `printer_type` | `mfp` (default), `inkjet`, `laser` or `office`. |
| `cartridge_order` | `color` (default) keeps the order printed on the machine, black to yellow. `level` puts the emptiest first, which is what you want when the card is there to tell you what to buy. A supply that reports no level has nothing to sort on and goes last. |
| `counters_align` | `left` (default) or `center`. `center` moves the numbers only: the function names keep their column at the left, so the rows stay aligned with each other whatever their length. |
| `cartridge_rows` | How many rows the cartridges are laid out on: `auto` (default: one row up to five, half and half beyond), or `1`, `2`, `3`. A card spanning several dashboard columns is wide enough for eight inks in a row, and only you know that. The levels drawn inside the machine always balance themselves, since the bay's width comes from the drawing and not from the card. |
